...
#include "PorousFlowFullySaturatedMassTimeDerivativeVariableBiot.h"

#include "MooseVariable.h"

registerMooseObject("PorousFlowApp", PorousFlowFullySaturatedMassTimeDerivativeVariableBiot);

InputParameters
PorousFlowFullySaturatedMassTimeDerivativeVariableBiot::validParams()
{
  ...
  params.addParam<MaterialPropertyName>(
      "biot_modulus_name", "biot_modulus", "Name of material property for biot modulus.");//✅0402    
...
PorousFlowFullySaturatedMassTimeDerivativeVariableBiot::PorousFlowFullySaturatedMassTimeDerivativeVariableBiot(
  ...
  : TimeKernel(parameters),
    ...
    _biot_coefficient(getMaterialProperty<Real>(_biot_coef_name)),// ✅
    _biot_modulus_name(getParam<MaterialPropertyName>("biot_modulus_name")),// ✅
    _biot_modulus(getMaterialProperty<Real>(_biot_modulus_name)),
                                            ...
                                            : nullptr)
{
  if (_dictator.numComponents() != 1 || _dictator.numPhases() != 1)
    mooseError("PorousFlowFullySaturatedMassTimeDerivativeVariableBiot is only applicable to single-phase, "
...
Real
PorousFlowFullySaturatedMassTimeDerivativeVariableBiot::computeQpResidual()
{
  ...
  if (_includes_mechanical)
    volume += _biot_coefficient[_qp] * (*_strain_rate)[_qp];
...
Real
PorousFlowFullySaturatedMassTimeDerivativeVariableBiot::computeQpJacobian()
{
...
Real
PorousFlowFullySaturatedMassTimeDerivativeVariableBiot::computeQpOffDiagJacobian(unsigned int jvar)
{
...
Real
PorousFlowFullySaturatedMassTimeDerivativeVariableBiot::computeQpJac(unsigned int pvar)
{
  const unsigned phase = 0;
  Real volume = (_pp[_qp][phase] - _pp_old[_qp][phase]) / _dt / _biot_modulus[_qp];
  ...
  {
    volume += _biot_coefficient[_qp] * (*_strain_rate)[_qp];
    dvolume += _biot_coefficient[_qp] * (*_dstrain_rate_dvar)[_qp][pvar] * _grad_phi[_j][_qp];
  }
  if (_multiply_by_density)
