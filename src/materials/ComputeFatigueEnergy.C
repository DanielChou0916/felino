...
#include "ComputeFatigueEnergy.h"
#include "MathUtils.h"
#include <algorithm>
registerMooseObject("SolidMechanicsApp", ComputeFatigueEnergy);

InputParameters
ComputeFatigueEnergy::validParams()
{
...
ComputeFatigueEnergy::ComputeFatigueEnergy(const InputParameters & parameters)
  : Material(parameters), 
    ...
    _bar_psi(declareProperty<Real>(_bar_psi_name)),  // ✅ 
    _bar_psi_old(getMaterialPropertyOld<Real>(_bar_psi_name)),

    _acc_bar_psi_name(getParam<MaterialPropertyName>("acc_bar_psi_name")),  // ✅ 
    _acc_bar_psi(declareProperty<Real>(_acc_bar_psi_name)),  // ✅ 
    _acc_bar_psi_old(getMaterialPropertyOld<Real>(_acc_bar_psi_name)),

    _n(getOptionalMaterialProperty<Real>("material_constant_n")),
    _R(getOptionalMaterialProperty<Real>("load_ratio")),
...
void
ComputeFatigueEnergy::initQpStatefulProperties()
{
  _acc_bar_psi[_qp] = _acc_bar_psi_old[_qp];
}
void
ComputeFatigueEnergy::initialSetup()
...
void
ComputeFatigueEnergy::computeQpProperties()
{ 
  Real current_psi;
  ...
  if (_energy_calculation == "mean_load")
  {
    ...
    //Real e_min = *std::min_element(eigval.begin(), eigval.end());
    Real e_max = *std::max_element(eigval.begin(), eigval.end());
        
    //Real R = e_min / (e_max + 1e-12); //1e-12 for numerical stability;
  ...
  { 
    RankTwoTensor trial_stress = _elasticity_tensor[_qp] * _strain[_qp];
    RankTwoTensor eigvec;
    std::vector<Real> eigval(LIBMESH_DIM);
    RankFourTensor Ppos =
  ...
  // 
  Real D_constant = _multiply_by_degradation ? _D[_qp] : 1.0;
  ...
  {
    Real delta_energy = (_bar_psi[_qp] > _bar_psi_old[_qp]) ? (_bar_psi[_qp] - _bar_psi_old[_qp]) : 0.0;
  ...
  else if (_accumulation_mode == "FatigueICLA")
  {
      ...
      else
      {
        ...
        if (delta_N < 0.0)
          delta_N = 0.0;
        Real delta_energy = _bar_psi[_qp] * delta_N;
        _acc_bar_psi[_qp] = _acc_bar_psi_old[_qp] + delta_energy;
      }
  }
