...
#include "PorousFlowPorosityVarBiot.h"

registerMooseObject("PorousFlowApp", PorousFlowPorosityVarBiot);

InputParameters
PorousFlowPorosityVarBiot::validParams()
{
  ...
  params.addParam<MaterialPropertyName>(
      "porosity_name", 
      "PorousFlow_porosity_output", 
      "Name of the porosity material property for output or transfer.");//✅
  params.addClassDescription("This Material calculates the porosity PorousFlow simulations");
  return params;
}

PorousFlowPorosityVarBiot::PorousFlowPorosityVarBiot(const InputParameters & parameters)
  : PorousFlowPorosityExponentialBase(parameters),
    ...
    _biot(getMaterialProperty<Real>(_biot_coef_name)),// ✅
    ...
    _porosity_output(declareProperty<Real>(_porosity_name))//✅
{
  if (_thermal && !isParamValid("thermal_expansion_coeff"))
    mooseError("PorousFlowPorosityVarBiot: When thermal=true you must provide a thermal_expansion_coeff");
  if (_fluid && !isParamValid("solid_bulk"))
    mooseError("PorousFlowPorosityVarBiot: When fluid=true you must provide a solid_bulk");
  if (_chemical && _num_c_ref != _dictator.numAqueousKinetic())
    mooseError("PorousFlowPorosityVarBiot: When chemical=true you must provide the reference_chemistry "
  ...
  if (_chemical && _num_initial_c != _dictator.numAqueousKinetic())
    mooseError("PorousFlowPorosityVarBiot: When chemical=true you must provide the "
               "initial_mineral_concentrations.  "
  ...
  if (_chemical && _c_weights.size() != _dictator.numAqueousKinetic())
    mooseError(
        "PorousFlowPorosityVarBiot: When chemical=true you must provde the correct number of "
  ...
  if (_nodal_material)
    _porosity_ptr = &getMaterialProperty<Real>("PorousFlow_porosity_nodal");//✅
  else
    _porosity_ptr = &getMaterialProperty<Real>("PorousFlow_porosity_qp");//✅
}

Real
PorousFlowPorosityVarBiot::atNegInfinityQp() const
{
  /*
   *
...
Real
PorousFlowPorosityVarBiot::datNegInfinityQp(unsigned pvar) const
{
  Real result = 0.0;
...
Real
PorousFlowPorosityVarBiot::atZeroQp() const
{
...
Real
PorousFlowPorosityVarBiot::datZeroQp(unsigned pvar) const
{
  Real result = 0.0;
  if (_chemical && (_t_step >= 1 || _app.isRestarting()))
    for (unsigned i = 0; i < _num_c_ref; ++i)
...
Real
PorousFlowPorosityVarBiot::decayQp() const
{
  Real result = 0.0;
  ...
  if (_fluid)
    result += _biot[_qp] * ((*_pf)[_qp] - _p_reference[_qp]);
...
Real
PorousFlowPorosityVarBiot::ddecayQp_dvar(unsigned pvar) const
{
  Real result = 0.0;
  ...
  if (_fluid)
    result += _biot[_qp] * (*_dpf_dvar)[_qp][pvar];

  return result;
}

RealGradient
PorousFlowPorosityVarBiot::ddecayQp_dgradvar(unsigned pvar) const
{
...
void//✅
PorousFlowPorosityVarBiot::computeQpProperties()//✅
{
  _porosity_output[_qp] = (*_porosity_ptr)[_qp];
}
